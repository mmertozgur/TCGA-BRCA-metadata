...
```{r}
surv_CD8A_filtered = select(BRCA_metadata,c(patient,days_to_last_follow_up,days_to_death,vital_status,
                                   CD8A,definition))
surv_CD8A_filtered = filter(surv_CD8A_filtered, definition == "Primary solid Tumor")

median_CD8A_filtered = median(surv_CD8A_filtered$CD8A, na.rm = TRUE)

surv_CD8A_filtered = surv_CD8A_filtered %>% 
  mutate(CD8A_category = case_when(
    CD8A >= median_CD8A_filtered ~ "high" , 
    CD8A < median_CD8A_filtered ~ "low"
  )) %>% 
  mutate(days_to_event = case_when(
    is.na(days_to_death) == FALSE ~ days_to_death,
    TRUE ~ days_to_last_follow_up
...
fit_CD8A_filtered <- survfit(Surv(days_to_event,status) ~ CD8A_category, data = surv_CD8A_filtered)
ggsurvplot(fit_CD8A_filtered,
           data=surv_CD8A_filtered,
           risk.table = TRUE,
           pval = TRUE,
           conf.int = FALSE,
           break.time.by = 1000,
